...
void Playlist::removeSong(int index) {
   if (index < 0 || index >= size) return;
    ...
    // Nếu bài đang phát bị xóa, tự động "phát" bài kế tiếp (tăng play_count)
    if (currentRemoved) {
#ifdef USE_THREADED_AVL
        if (hasCurrent && currentIt != songs.endIt()) {
            currentIt.value()->increasePlayCount();
        }
#else
        if (currentIndex != -1) {
            Song* newCurrent = getSong(currentIndex);
            if (newCurrent) {
                newCurrent->increasePlayCount();
            }
        }
#endif
    }
